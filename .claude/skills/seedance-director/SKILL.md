...
## PROJECT ADAPTATION — Arrival Story hero video (this repo)

When generating shots for the Arrival Story landing-page hero, these overrides apply
on top of everything above. Source of truth: `Lily-Hero-Video-Plan.md` (§3 cast,
§4 storyboard + cue table, Appendix A character sheets).

**Reference-image legend (fixed across the whole production):**
- `<<<image_1>>>` — Sarah character sheet
- `<<<image_2>>>` — Marco character sheet
- `<<<image_3>>>` — Janet character sheet
- `<<<image_4>>>` — Emma character sheet
- `<<<image_5>>>` — Lisa character sheet
- `<<<image_6>>>`+ — location plates for the scene, in the order attached

**Duration:** default 5 seconds per shot, hard cap 6. Quick-cut beats (decision
triptych, montage flicker, split panels) still generate at 3–5s — the edit trims.
Never generate a multi-scene sequence in one prompt; one storyboard shot = one
prompt pair.

**Style & Mood defaults (unless the scene says otherwise):** cinematic realism,
shallow depth of field, lifted blacks, gentle film grain. Color script by act:
golden-hour warmth (bedroom/home) → bright warm daylight (labor begins) → dusk
(decision) → cool clinical-but-comforting (hospital exteriors/corridors) → low warm
night interior (game time) → cool gray drizzle (Seattle) → blush warmth (birth) →
deep plum quiet (keepsake). Footage is silent — omit the Audio section always.

**Project hard rules (override anything that conflicts):**
- **No rendered newborn faces.** Baby Lily appears only as: swaddle from behind, a
  tiny hand gripping a finger, out-of-focus warmth.
- **Phone screens are always dim, dark, or angled away** — never readable. The real
  UI lives in the page overlay, not the footage.
- **Sarah is never weak.** No wheelchairs, no distress closeups, no medical detail.
  Laboring Sarah is breathing, steadying, walking under her own power.
- **No mirror reflections rendered.** Scene 1 Shot A uses the camera-as-mirror
  cheat: camera faces Sarah directly as she raises the phone toward the lens.
- **During labor scenes, Marco operates the phone, not Sarah.**
- Single-character shots wherever the storyboard allows (split-screen panels are
